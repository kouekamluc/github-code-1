...
function renderAreaProfile(area = selectedArea) {
  ...
  areaProfile.innerHTML = `
    <div class="profile-hero">
      ...
      <span class="priority-badge">${escapeHtml(localPriority?.priority_label || 'Watch')}</span>
    </div>
    ...
    <div class="profile-notes">
      ...
      ${localPriority?.priority_score >= 78 ? 'Immediate field validation and maintenance review.' : localPriority?.priority_score >= 55 ? 'Schedule survey and monitor assets.' : 'Keep in watchlist and enrich with field data.'}
    </div>
  `;
}
...
function renderAssets() {
  document.getElementById('assets-list').innerHTML = assets.map(asset => `
    <article class="list-card status-${escapeHtml(asset.status)}">
      <div><strong>#${asset.id} ${escapeHtml(asset.name)}</strong><span>${escapeHtml(asset.asset_type)} · ${escapeHtml(asset.commune)}, ${escapeHtml(asset.department)}</span></div>
      <span class="status-pill">${escapeHtml(asset.status)}</span>
...
function renderReports() {
  document.getElementById('reports-list').innerHTML = reports.map(report => `
    <article class="list-card">
      <div><strong>${escapeHtml(report.report_type)}</strong><span>${escapeHtml(report.commune)}, ${escapeHtml(report.department)} · ${escapeHtml(report.submitted_by)}</span></div>
...
function renderAlerts() {
  document.getElementById('alerts-list').innerHTML = alerts.map(alert => `
    <article class="list-card severity-${escapeHtml(alert.severity)}">
      <div><strong>${escapeHtml(alert.title)}</strong><span>${escapeHtml(alert.severity)} · ${escapeHtml(alert.status)}</span></div>
...
function renderPriority() {
  document.getElementById('priority-list').innerHTML = priorityZones.slice(0, 18).map(zone => `
    <article class="list-card">
      <div><strong>${escapeHtml(zone.commune)}</strong><span>${escapeHtml(zone.department)}, ${escapeHtml(zone.region)} · ${formatNumber(zone.population)} people</span></div>
      <span class="priority-badge">${escapeHtml(zone.priority_label)} ${zone.priority_score.toFixed(0)}</span>
      <p>${zone.open_alert_count} open alerts · ${zone.asset_count} monitored assets · ${zone.report_count} field reports · ${formatRate(zone.phone_rate)} phone ownership</p>
    </article>
  `).join('');
}
...
function renderIot() {
  document.getElementById('iot-list').innerHTML = readings.map(reading => `
    <article class="list-card">
      <div><strong>${escapeHtml(reading.reading_type)}</strong><span>Asset #${reading.asset_id} · ${escapeHtml(reading.created_at)}</span></div>
...
function renderDecision(report) {
  document.getElementById('decision-report').innerHTML = `
    ...
    <div class="list-stack">
      ${report.top_priority_zones.map(zone => `<article class="list-card"><strong>${escapeHtml(zone.commune)}</strong><span>${escapeHtml(zone.department)}, ${escapeHtml(zone.region)} · Score ${zone.priority_score.toFixed(0)}</span></article>`).join('')}
    </div>
  `;
}
